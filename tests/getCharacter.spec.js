...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    const sensitive1 = getCharacter('Arya');
    const sensitive2 = getCharacter('ArYa');
    const sensitive3 = getCharacter('ARYA');
    expect(sensitive1).toEqual(sensitive2);
    expect(sensitive1).toEqual(sensitive3);
    expect(sensitive2).toEqual(sensitive3);
    // 6. Teste se ao passar um nome que não está na tabela, a função retorna undefined.
    expect(getCharacter('Jadson Mattos')).toBeUndefined();
  });
});
